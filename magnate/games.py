...
class GameManager:
    ...
    def _end_game_logic(cls, game: Game, user: CustomUser, action: Optional[Action] = None) -> Response:
        ...
        if not game.finished:
            ...
            final_money_dict_pk = {pk: final_money_dict[username] for pk, username in pk_to_username.items()}

            # last_money = dinero final sin contar los bonuses
            last_money_dict = {
                pk: amount - bonus_per_player.get(pk, 0)
                for pk, amount in final_money_dict_pk.items()
            }
    ...
    def _set_next_phase_timer(game: Game, user: CustomUser):
        ...
        if Bot.objects.filter(pk=user.pk).exists():
            bot_play_callback.apply_async(args=[game.pk, user.pk], countdown=random.randint(5, 8))
